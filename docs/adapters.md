...
case (Sec. 8.4), that's 16,667 regions each re-splitting the *entire file*
...
own Sec. 5.4 trailing-backslash regex (`/(?<!\\)\\$/`, a one-character
...
(`languages/markdown/continuation-prefix.ts`, Sec. 5.3's table as a direct
...
command), while `latexContinuationPrefix` (Sec. 6.3) deliberately derives a
...
internal spaces exactly like prose), so Sec. 4.3's `extraUnbreakable`
