...
- **HTML** (+ erb/svelte/vue aliases) -- Comments: single `<!-- -->` form,
  ...
  region (a la Markdown paragraphs) is a real scope question this pass
...
|---|---|---|---|---|---|---|
| TOML | both | Confirmed working (local build, no Emscripten/Docker) | Pure descriptor (comment-only) | No | ~0.2x (Python-shape simple; string-value wrapping deliberately out of scope) | **High** -- trivial, dogfoodable via this project's own `pyproject.toml`/`.rewraprc` convention |
| Shell script (Bash) | both | Confirmed working, prebuilt | Pure descriptor (comment-only); full strings need engine work (heredocs) | No | ~0.2x comment-only; string support unscoped for now | **High** -- near-universal on Windows/WSL/Linux; comment-only ships cheaply |
| CSS | both | Confirmed working, prebuilt | Pure descriptor | No | ~0.3x (simpler than C++'s plainBlock case) | **High** -- lowest technical risk found in this whole pass, real ecosystem demand |
| SCSS | both | Confirmed working (local build) | Pure descriptor (Java's two-comment-type mechanism reused verbatim) | No | ~0.35x | **High** -- same low risk as CSS, mechanism already proven by Java |
| PowerShell | both | Confirmed working, prebuilt | Comment-only likely pure descriptor + new dialect id; full strings need engine work | Yes (comment-based help) | ~0.5x comment-only | **High** -- the one candidate directly Windows-relevant given Jarin's OS |
| Dart | both | Prebuilt WASM **does not load** with pinned runtime; local rebuild fixes it (Finding D) | Comment-only pure descriptor + new dialect id (cleanest node shape of the 13); full strings need engine work | Yes (Dartdoc) | ~0.6x comment-only | Medium -- clean shape, but no signal Jarin's stack touches Dart/Flutter |
| C# | both | Confirmed working, prebuilt | Comment-only pure descriptor + new dialect id; full strings need engine work (interpolation + doubling-escape) | Yes (XmlDoc) | ~1x (comparable to 12b's actual, not predicted, scope) | Medium -- very popular language generally, no direct signal from this project's own context |
| PHP | both | Confirmed working, prebuilt | Comment-only pure descriptor + new dialect id (reusing jsdoc-shaped logic); full strings need engine work (interpolation + heredoc/nowdoc) | Yes (phpdoc, structurally reuses jsdoc parsing) | ~1x (comparable to 12b) | Medium -- huge install base, but a style of project unlikely to be Jarin's own |
...
| HTML (+erb/svelte/vue) | both | Confirmed working, prebuilt | Pure descriptor (comment-only, matching Rewrap's own scope choice) | No | ~0.3x | Low-medium -- trivial once scoped to comments, but templating-language text-node wrapping (if ever wanted) is unexplored |
| C/C++ family cousins (Objective-C, Verilog/SystemVerilog, Groovy, Scala) | both | Prebuilt `.wasm` confirmed at registry level, not dry-run parsed | Provisional: likely pure descriptor (Java-style comment precedent already proven 4x) | Likely no (Javadoc/Doxygen probably already cover these ecosystems) | Provisional ~0.3-0.5x each | Low -- no signal these are part of Jarin's stack; would need real probing before scoring further |
