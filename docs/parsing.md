...
- **One real discrepancy from the plan's own node-types.json reading:**
  ...
  This doesn't change anything downstream, because Sec. 5.2 already hedged
...
- Tabs in container prefixes (`-\t`, `>\t`) are preserved verbatim in both
  ...
  logic (Sec. 5.3) is the only thing that will handle this.
- CRLF: a paragraph's own span (and its `inline` child's span) reproduces
  ...
  from "end of that source line's content, `\r` excluded" (as Sec. 5.2
...
All four were open questions in Sec. 2.1; all four now verified rather than
inferred from the README.

**Setext heading exclusion (Sec. 3.3 item 3):** confirmed trivial --
...
**Hard-break backslash nuance for Sec. 5.4, worth carrying into Phase C:** the
...
try** -- this was the single biggest open question in the plan (Sec. 2.2/Sec. 3.1:
...
**The `.github/workflows/grammar-wasm.yml` Linux-builder fallback (Sec. 3.1)
...
Sec. 2.1's advance finding):** `latex-lsp/tree-sitter-latex`'s GitHub releases
...
from the Markdown asset's GitHub-attested one, exactly as Sec. 3.1 specifies.

**Sec. 6.1/Sec. 6.2 node shapes, all probed directly:**
...
- **Sectioning node extent -- the one the plan called out as
  ...
  paragraph-level node anywhere in this grammar, confirming Sec. 1's claim
...
- **New node type the plan didn't anticipate:** `\newtheorem{name}[counter]{text}`
  ...
  (whole-line-command detection is line-scan-based per Sec. 6.2 regardless),
  but worth knowing the commandRegex-vs-tree fallback in Sec. 6.2 ("when the
...
- Nested braces resolve correctly through the tree:
  ...
  regex -- is the right tool when Sec. 6.2's `commandRegex` and the tree
  disagree.
- **`\verb`/`\lstinline` are genuinely unprotected by this grammar** --
  ...
  exactly like ordinary prose. Sec. 4.3's `extraUnbreakable` regex for
...
- **`\par` has no special tree signal** -- confirmed: alone on its own
  ...
  break of any kind in the tree. Sec. 6.2's line-scan-based
  structural-line detection is doing all the real work here; the tree
  contributes nothing extra for this case.
- **Trailing `%` comment mid-line -- the one real semantic hazard (Sec. 6.4)**
  ...
  right for free, which is the whole argument in Sec. 6.2 for using
...
**Environment classification, completed for Sec. 6.2's full preserve-list
candidates** -- probed via `docs/spikes/tree-sitter-latex-probe2.mjs`,
following up on the subset above with every remaining name Sec. 6.2 needed
...
- `lstlisting` gets its **own** dedicated `listing_environment` node type
  ...
  `generic_environment` like the rest of the unnamed cases. Sec. 6.2's mask
...
- `Verbatim`, `BVerbatim`, `alltt`, `tikzpicture`, `tabular`, `tabular*`,
  ...
  excluded from it, per Sec. 6.2's own worked example).
...
document (that file repeated 50x) also parsed with zero errors, in 39.8 ms
...
check Sec. 8.4 calls for against multiple real papers (that's explicitly
