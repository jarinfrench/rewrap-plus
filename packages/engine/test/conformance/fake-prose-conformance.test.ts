 ...
 * contract Sec. 3.2 describes for a real adapter (a "lazy" line with no
...
function discoverBlockCommentProse(source: string, languageId: string): WrappableRegion[] {
  ...
  for (let row = openRow + 1; row < closeRow; row++) {
    ...
    // contract Sec. 3.2 describes) is handled the same way Markdown's is:
  ...
  return [
    {
      ...
      // *derived from* indentColumn for exactly this reason (Sec. 5.3), so a
