...
/**
 * The hard gate Phase 6b exists for: this is the second adapter through
 * the identical conformance suite Python already passes
 * (`./python-conformance.test.ts`). Every source below exercises both
 * comment forms this canary supports — `//` and JSDoc-style block
 ...
 *
 * If this file's suite passes, per the plan's own hard gate: no engine
 * change was needed to add this adapter beyond what was already made,
 * ahead of time, in the three commits before the canary itself
 * (comment dissolve/emit promoted to generic modules, codeLikeKeywords
 * split into descriptor data, wrapRegions generalized). Those *were*
 * engine changes the canary's design surfaced — recorded, as the plan
 * asks, in `docs/adapters.md`.
 */
const CRLF_SOURCE =
