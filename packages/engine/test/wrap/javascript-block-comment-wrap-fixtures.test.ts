...
import plainIn from '../fixtures/javascript/block-comments/001-plain-narrative.in.js?raw';
import plainOut from '../fixtures/javascript/block-comments/001-plain-narrative.out.js?raw';
...
describe('JavaScript blockComment (plain /* */) wrapping -- end-to-end gold fixtures', () => {
  ...
  it('produces no line over the column limit', async () => {
    const result = await wrapRegions(plainIn, 'javascript', 'all', config(), parserManager);
  ...
  it('is idempotent: re-wrapping the gold output produces no further edits', async () => {
    const result = await wrapRegions(plainOut, 'javascript', 'all', config(), parserManager);
