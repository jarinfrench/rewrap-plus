 ...
 * piece of Python-line-comment-specific state that doesn't belong on the
 ...
 * `./adapter.ts`'s `groupRegions`) as either a directive line to carry
 * through verbatim, or reflowable content with its marker stripped.
 *
 * Directive detection tests `descriptor.comments.neverReflow` against
 * each line's *raw* text (shebangs, coding declarations, `# type:`,
 * `# noqa`, `# pylint:`, `# fmt:` — see `pythonDescriptor`): "moving one
 * of these to a different line can change program behavior," so they're
...
export function dissolveLineComments(
  ...
  const flushReflowable = (): void => {
    ...
    if (looksLikeCommentedOutCode(contents)) {
      // Preserve the block exactly as it read in source, `#`/spacing
      // and all — reflowing commented-out code risks producing text
      // that no longer round-trips to valid Python if uncommented.
      blocks.push({
        type: 'verbatim',
