...
/**
 * `\verb`/`\lstinline` -- Sec. 4.3's true never-split spans: unlike `$...$`
 * (breakable in TeX; deliberately not added, Sec. 11), a split inside either
 ...
 * Sec. 6.3) -- read via `sliceSpanText`, not a fresh `source.split('\n')`, for
 ...
 * fresh). For an ordinary paragraph the two columns coincide (Sec. 6.3), so
